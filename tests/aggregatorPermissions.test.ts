...
describe("aggregator permission matrix", () => {
  ...
  it("every permission except the deliberately Owner/Admin-only ones is delegated to at least one other role", () => {
    // aggregator.keys.manage (production API credentials) and
    // aggregator.team.manage (who else gets access at all) are the two
    // categories intentionally NOT delegated below Owner/Admin — minting a
    // money-moving API key or adding a new staff member are the org's own
    // accountable-principal decisions, not something Operations/Finance
    // should be able to do unilaterally.
    const OWNER_ADMIN_ONLY: AggregatorPermission[] = ["aggregator.keys.manage", "aggregator.team.manage"];
