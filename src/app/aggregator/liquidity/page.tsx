  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-[var(--foreground)]">Network Liquidity & Float Command</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div className="p-5 rounded-3xl bg-[var(--surface)] border border-[var(--border)] space-y-1">
          <div className="text-[10px] font-mono text-[var(--foreground-muted)] uppercase">Aggregator Main Float</div>
          <div className="text-2xl font-black font-mono text-amber-600 dark:text-amber-400">
            {formatCurrency(liquidity.aggregatorMainWallet)}
          </div>
          <div className="text-[10px] text-teal-600 dark:text-teal-300 font-mono">Available for dispatch</div>
        </div>

        <div className="p-5 rounded-3xl bg-[var(--surface)] border border-[var(--border)] space-y-1">
          <div className="text-[10px] font-mono text-[var(--foreground-muted)] uppercase">Distributed Agent Float</div>
          ...
          <div className="text-[10px] text-[var(--foreground-muted)] font-mono">Across {agents.length} active agents</div>
        </div>

        <div className="p-5 rounded-3xl bg-[var(--surface)] border border-[var(--border)] space-y-1">
          <div className="text-[10px] font-mono text-[var(--foreground-muted)] uppercase">Network Drawer Cash</div>
          <div className="text-2xl font-black font-mono text-[var(--foreground)]">
            {formatCurrency(liquidity.estimatedCashInNetworkDrawer)}
          </div>
          <div className="text-[10px] text-[var(--foreground-muted)] font-mono">Physical cash-in reserves</div>
        </div>

        <div className="p-5 rounded-3xl bg-[var(--surface)] border border-[var(--border)] space-y-1">
          <div className="text-[10px] font-mono text-[var(--foreground-muted)] uppercase">Float Health Status</div>
      ...
      <div className="rounded-3xl bg-[var(--surface)] border border-[var(--border)] overflow-hidden shadow-xl">
        ...
        <div className="divide-y divide-[var(--border)]">
          {lowFloatAgents.map((agt) => (
            <div
              key={agt.id}
              className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-[var(--surface-2)] transition-colors"
            >
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-[var(--foreground)] text-sm">{agt.fullName}</span>
                  <span className="px-2 py-0.2 rounded text-[10px] font-mono font-bold bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20">
                    {agt.agentCode}
                  </span>
                  <span className="text-xs text-[var(--foreground-muted)] font-mono">• {agt.territoryName}</span>
                </div>
                <div className="text-xs text-[var(--foreground-muted)]">{agt.businessName} • {agt.phone}</div>
              </div>

              <div className="flex items-center gap-4 self-start sm:self-auto">
                <div className="text-right">
                  <div className="text-[10px] font-mono text-[var(--foreground-muted)] uppercase">Current Float</div>
                  <div className="text-sm font-bold font-mono text-rose-600 dark:text-rose-400">{formatCurrency(agt.walletBalance)}</div>
                </div>
