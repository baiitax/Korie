...
export default function AggregatorTargetsPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
      {/* Header */}
      <div>
        <h1 className="text-xl sm:text-2xl font-black text-[var(--foreground)]">Aggregator Targets & Milestones</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {targets.map((tgt) => {
          ...
          return (
            <div
              key={tgt.id}
              className="p-6 rounded-3xl bg-[var(--surface)] border border-[var(--border)] space-y-4 flex flex-col justify-between"
            >
              <div>
                <div className="flex items-start justify-between">
                  ...
                  <span className="px-2.5 py-1 rounded-xl text-xs font-mono font-bold bg-[var(--surface-2)] text-[var(--foreground)]">
                    {tgt.period}
                  </span>
                </div>

                <h3 className="font-bold text-[var(--foreground)] text-base mt-3 leading-snug">{tgt.title}</h3>
                <div className="text-xs text-[var(--foreground-muted)] font-mono mt-1">Deadline: {tgt.deadline}</div>
              </div>
              ...
              <div className="space-y-2 pt-2 border-t border-[var(--border)]">
                ...
                <div className="flex justify-between text-xs">
                  <span className="text-[var(--foreground-muted)]">Achieved:</span>
                ...
                <div className="space-y-1 pt-1">
                  ...
                  <div className="text-right text-[10px] font-mono text-teal-600 dark:text-teal-300 font-bold">{progress}% Completed</div>
                </div>
              </div>
            </div>
          );
