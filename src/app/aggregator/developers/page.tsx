  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-[var(--foreground)]">Aggregator API Gateway & Webhooks</h1>
      ...
      <div className="p-6 rounded-3xl bg-[var(--surface)] border border-[var(--border)] space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-[var(--foreground)] text-base">API Authentication Credentials</h3>
            <p className="text-xs text-[var(--foreground-muted)]">Never expose aggregator secret keys in client-side code.</p>
          </div>
        </div>

        <div className="p-4 rounded-2xl bg-[var(--surface-2)] border border-[var(--border)] space-y-3">
          <div className="flex items-center justify-between">
            <span className="font-bold text-[var(--foreground)] text-xs">Production Aggregator Key</span>
            <span className="text-[10px] font-mono text-[var(--foreground-muted)]">Last used: 2 mins ago</span>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center justify-between p-2 rounded-xl bg-[var(--background)] border border-[var(--border)] mt-1">
              <span className="font-mono text-xs text-[var(--foreground)] truncate">pk_live_agg_9948102938102938</span>
              <button
              ...
              >
                {copiedKey === "pub" ? <Check className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center justify-between p-2 rounded-xl bg-[var(--background)] border border-[var(--border)] mt-1">
              <span className="font-mono text-xs text-teal-600 dark:text-teal-300 truncate">
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  ...
                  className="p-1 rounded bg-[var(--surface-2)] text-[var(--foreground-muted)] hover:text-[var(--foreground)]"
                >
                  {copiedKey === "sec" ? <Check className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                </button>
              </div>
            </div>
          </div>
      ...
      <div className="p-6 rounded-3xl bg-[var(--surface)] border border-[var(--border)] space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-[var(--foreground)] text-base">cURL Quickstart: Dispatch Float to Agent</h3>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-[var(--background)] font-mono text-xs text-[var(--foreground)] border border-[var(--border)] overflow-x-auto space-y-1">
          <div className="text-[var(--foreground-muted)]"># Direct float injection from Aggregator Wallet to Agency Cash Point</div>
          <div className="text-teal-600 dark:text-teal-400">curl -X POST https://api.koriepay.com/v1/aggregator/liquidity/dispatch \</div>
          <div className="pl-4 text-[var(--foreground)]">-H "Authorization: Bearer kp_live_agg_••••••••" \</div>
          <div className="pl-4 text-[var(--foreground)]">-H "Content-Type: application/json" \</div>
          <div className="pl-4 text-[var(--foreground)]">-d '&#123;</div>
          ...
          <div className="pl-4 text-[var(--foreground)]">&#125;'</div>
        </div>
      </div>
    </div>
  );
