...
/** POST /api/admin/operator-keys/[id]/rotate — 24h grace on the old secret. */
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = engine.rotateCredential(id, guard.context.userId || 'admin-console');
    return NextResponse.json({ success: true, data: { credential: result.credential, secretKeyRaw: result.secretKeyRaw } });
  } catch (err: unknown) {
